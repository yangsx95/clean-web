...
- **安全搜索**：可独立开启或关闭。
- **严格模式**：可独立开启或关闭；开启后追加更激进的高风险域名后缀、关键词和 IP/CIDR 拦截规则。
- **广告与跟踪保护**：可独立开启或关闭。
- **短视频与游戏娱乐**：可独立开启或关闭；开启后拦截常见短视频、直播和游戏平台域名，以及被归入该分类的订阅规则。
- **访问日志**：可独立开启或关闭，默认开启。
- **自动选择节点**：可独立开启或关闭。
...
广告和普通跟踪器合并为一个可选类别；短视频、直播、游戏平台等娱乐内容作为另一个可选类别，由管理者按家庭规则开启；恶意广告仍属于强制安全类别。
...
严格模式只基于可观察的域名、DNS 查询和目标地址生效，不检查 HTTPS URL 路径或网页正文。严格模式规则允许更高误杀率，用于拦截含高风险关键词、明确高风险后缀或已知高风险地址段的目标；不得默认使用随机字符串、长域名等宽泛启发式正则直接拦截正常基础设施域名。
...
安全搜索不再通过 Mihomo hosts 或 DNS 映射实现，也不作为规则订阅格式导入。后续新增浏览器或搜索服务时应扩展浏览器策略模块，而不是把厂商安全搜索映射写入内核配置。
...
- 支持管理者为特定域名、IP 或 CIDR 设置直连、走代理或按系统路由；按系统路由表示过滤允许后交回操作系统路由表处理，不保证一定命中特定第三方 VPN。未命中路由规则的允许流量使用当前默认代理策略。
