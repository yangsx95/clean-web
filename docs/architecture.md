...
CleanWeb 将策略管理和网络执行分离：CleanWeb 是唯一策略权威，Mihomo 是受控的 TUN、DNS、安全搜索和代理执行内核。代理订阅不能修改 DNS、TUN、过滤规则、系统路由或绕过策略。过滤决策必须先于代理路由。
...
安全搜索由 Mihomo 配置完成：CleanWeb 生成顶层 `hosts` 映射，启用 `dns.use-hosts`，通过 TUN `dns-hijack` 接管 53 端口 DNS，并将安全搜索域名及目标加入 `fake-ip-filter` 和 `sniffer.skip-domain`，避免 fake-ip 或 SNI 目标覆盖破坏厂商安全搜索别名。CleanWeb 不修改系统 hosts，也不在业务代码中写死厂商 IP。
