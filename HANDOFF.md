...
Expected current results: 13 Rust tests and 6 frontend tests pass.

## Not implemented yet

1. Proxy payload encryption. Sanitized proxy YAML currently lands in SQLite and
   may contain credentials. Before production, encrypt it with a key protected
   by macOS Keychain; never write credentials to logs.
2. Complete canonical node parsing for URI-list subscriptions. Clash YAML retains
   all Mihomo-supported node types, but URI lists are currently only recognized
   and counted.
3. Mihomo binary packaging and lifecycle management.
4. Generated locked Mihomo configuration and control API integration.
5. Proxy node/group selection and actual latency testing.
6. Privileged macOS helper/network extension for TUN and route/DNS changes.
7. fake-IP DNS, DNS hijacking, DoH/DoT bypass handling, and safe-search mapping.
8. Real traffic decisions wired from normalized rules into Mihomo.
9. Real access log ingestion, retention cleanup, filtering, and export.
10. Detection and warning for another VPN/TUN interface.
11. Protection continuing after the UI window/process exits.
12. Custom parent black/white rule CRUD UI.
13. Signed automatic rule packages and license/source catalog.
14. Intel build and universal single DMG.
15. Apple Developer signing, notarization, and macOS 13 Intel/ARM end-to-end tests.
...
1. Encrypt persisted sanitized proxy payloads using a Keychain-protected key.
2. Finish canonical proxy node/group persistence and tests.
3. Fetch and verify both official Mihomo binaries.
4. Implement a `mihomo` Rust module for config generation, process lifecycle,
   control API health, and latency tests without enabling TUN yet.
5. Implement the privileged macOS helper/network-extension design; do not fake
   protection by merely toggling the persisted switch.
6. Wire TUN, fake-IP DNS, generated blocking rules, logs, and failure recovery.
7. Add universal targets and run the requirement-by-requirement completion audit.
